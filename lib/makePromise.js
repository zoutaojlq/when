...
define(function() {
	...
	return function makePromise(environment) {
		...
		Promise.prototype.then = function(onFulfilled, onRejected, onProgress) {
			...
			parent.when({ resolve: child.resolve, notify: child.notify, context: child,
				receiver: parent.receiver,
				f: onFulfilled, r: onRejected, u: onProgress });
		...
		function all(promises) {
			/*jshint maxcomplexity:7, maxdepth:4*/
		...
		Handler.prototype.chain = function(to, f, r, u) {
			this.when({ resolve: noop, notify: noop, receiver: to, f: f, r: r, u: u });
		};
		...
		DeferredHandler.prototype.when = function(continuation) {
			...
			if(this.resolved) {
				tasks.enqueue(new RunHandlerTask(continuation, this.handler));
			} else {
		...
		AsyncHandler.prototype.when = function(continuation) {
			tasks.enqueue(new RunHandlerTask(continuation, this.join()));
		};
		...
		FulfilledHandler.prototype.when = function(continuation) {
			...
			var x = typeof continuation.f === 'function'
				? tryCatchReject(continuation.f, this.value, continuation.receiver)
				: this.value;
		...
		RejectedHandler.prototype.when = function(continuation) {
			...
			var x = typeof continuation.r === 'function'
				? tryCatchReject(continuation.r, this.value, continuation.receiver)
		 ...
		 * @constructor
		 */
		function RunHandlerTask(continuation, handler) {
		...
		ProgressTask.prototype._notify = function(continuation) {
			var x = typeof continuation.u === 'function'
				? tryCatchReturn(continuation.u, this.value, continuation.receiver)
				: this.value;
