...
define(function() {
	...
	return function makePromise(environment) {
		...
		Promise.prototype.then = function(onFulfilled, onRejected, onProgress) {
			...
			from.when(to.resolve, to.notify, to, from.receiver, nil, onFulfilled, onRejected, onProgress);

			return new InternalPromise(to);
		};

		Promise.prototype.fold = function(f, x) {
			var from = this._handler;
			var to = new DeferredHandler(from.receiver);
			from.when(to.resolve, to.notify, to, from.receiver, x, f);
		...
		function all(promises) {
			...
			function resolveOne(resolver, results, handler, i) {
				handler.when(noop, noop, void 0, resolver, results, function(results, x) {
					results[i] = x;
					if(--pending === 0) {
		...
		function race(promises) {
			...
			for(var i=0; i<promises.length; ++i) {
				getHandler(promises[i]).when(noop, noop, void 0, h, nil, h.resolve, h.reject);
			}
		...
		DeferredHandler.prototype.run = function() {
			...
			for (var i = 0; i < q.length; i+=8) {
				handler.when(q[i], q[i+1], q[i+2], q[i+3], q[i+4], q[i+5], q[i+6], q[i+7]);
			}
		};
		...
		DeferredHandler.prototype.when = function(resolve, notify, t, receiver, x, f, r, u) {
			if(this._isMonitored()) {
				this.context = void 0;
			}

			if(this.resolved) {
				tasks.enqueue(new RunHandlerTask(resolve, notify, t, receiver, x, f, r, u, this));
			} else {
				this.consumers.push(resolve, notify, t, receiver, x, f, r, u);
			}
		};
		...
		AsyncHandler.prototype.when = function(resolve, notify, t, receiver, x, f, r, u) {
			tasks.enqueue(new RunHandlerTask(resolve, notify, t, receiver, x, f, r, u, this.join()));
		};
		...
		BoundHandler.prototype.when = function(resolve, notify, t, receiver, x, f, r, u) {
			...
			this.join().when(resolve, notify, t, receiver, x, f, r, u);
		};
		...
		ThenableHandler.prototype.when = function(resolve, notify, t, receiver, x, f, r, u) {
			if(!this.assimilated) {
				this.assimilated = true;
				this._assimilate();
			}
			DeferredHandler.prototype.when.call(this, resolve, notify, t, receiver, x, f, r, u);
		};
		...
		FulfilledHandler.prototype.when = function(resolve, notify, t, receiver, x, f) {
			if(this._isMonitored()) { this._enterContext(); }

			var y = typeof f === 'function'
				? x === nil
						? tryCatchReject(f, this.value, receiver)
						: tryCatchReject2(f, x, this.value, receiver)
				: this.value;
		...
		RejectedHandler.prototype.when = function(resolve, notify, t, receiver, x, f, r) {
			if(this._isMonitored()) {
				this._removeTrace();
				this._enterContext();
			}

			var y = typeof r === 'function'
		 ...
		 * @constructor
		 */
		function RunHandlerTask(a, b, c, d, e, f, g, h, handler) {
			this.a=a;this.b=b;this.c=c;this.d=d;this.e=e;this.f=f;this.g=g;this.h=h;
			this.handler = handler;
		}

		RunHandlerTask.prototype.run = function() {
			this.handler.join().when(this.a,this.b,this.c,this.d,this.e,this.f,this.g,this.h);
		};
		...
		ProgressTask.prototype.run = function() {
			...
			for (var i = 1; i < q.length; i+=8) {
				this._notify(q[i], q[i+1], q[i+2], q[i+6]);
			}
		};
